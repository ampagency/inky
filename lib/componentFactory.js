...
module.exports = function(element) {
  ...
  switch (element[0].name) {
    ...
    case this.components.button:
      ...
      // Added support for MailChimp editor on buttons
      var mcAttr = '';
      if (element.attr('mc:edit')) {
        mcAttr = ' mc:edit="' + element.attr('mc:edit') + '"';
      }

      // If we have the href attribute we can create an anchor for the inner of the button;
      if (element.attr('href')) {
        inner = format('<a %s href="%s"%s>%s</a>', mcAttr, element.attr('href'), target, inner);
      }
